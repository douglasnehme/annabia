...
METADICT = {
'lago'           : { 'lon': -58.88, 'lat': -62.183, 'name': 'Glubokoe Deepe'                 },
'reanalise'      : { 'lon': -62.5 , 'lat': -62.5  , 'name': u'Reanálise 1\nNCEP/NCAR'        },
'bellingshausen' : { 'lon': -59.  , 'lat': -62.2  , 'name': 'Bellingshausen'                 },
'deception'      : { 'lon': -60.7 , 'lat': -63.   , 'name': 'Deception'                      },
'halley'         : { 'lon': -26.6 , 'lat': -75.58 , 'name': 'Halley'                         },
...
'valdivia'       : { 'lon': -73.1 , 'lat': -39.6  , 'name': 'Valdivia'                       },
...
m = Basemap(width = 330000, height = 220000, resolution = 'f', projection = 'laea',
            lat_ts = lat_0, lat_0 = lat_0, lon_0 = lon_0, ax = ax2)

m.drawparallels(np.arange(-63.5, -61.5, 0.5), labels = [False, True, False, False], 
                color = 'gray', linewidth = 0)
m.drawmeridians(np.arange(-63.0, -56.0, 1.0), labels = [False, False, True, True],
                color = 'gray', linewidth = 0)


m.drawcoastlines(linewidth = 0.5, zorder = 1)
m.fillcontinents(color = 'silver', zorder = 1)

m.drawmapscale(-57.5, -63.3, lat_0, lon_0, 60, barstyle = 'fancy')
...
m.scatter(METADICT['lago']['lon'], METADICT['lago']['lat'],
          s = 35, latlon = True, marker = '^', color = 'k', linewidths = 1.5, zorder = 3)

m.scatter(METADICT['reanalise']['lon'], METADICT['reanalise']['lat'],
          s = 50, latlon = True, marker = '*', color = 'k', linewidths = 1.5, zorder = 3)

m.scatter([ METADICT['bellingshausen']['lon'], METADICT['deception']['lon'] ],
          [ METADICT['bellingshausen']['lat'], METADICT['deception']['lat'] ],
          s = 35, latlon = True, marker = 's', color = 'k', linewidths = 1.5, zorder = 3)

ax2.annotate('(b)', (0.01, 0.95), xycoords = 'axes fraction', fontsize = 12)

ax2.annotate(u'Ilha Rei George', (0.785, 0.755), xycoords = 'axes fraction', color = 'w',
              fontsize = 8, horizontalalignment = 'center', zorder = 1, rotation = 11.5)

ax2.annotate(METADICT['lago']['name'], (0.8, 0.555), xycoords = 'axes fraction', horizontalalignment = 'center')

ax2.annotate(METADICT['reanalise']['name'], (0.115, 0.525), xycoords = 'axes fraction', horizontalalignment = 'center')

ax2.annotate(METADICT['deception']['name'], (0.398, 0.185), xycoords = 'axes fraction', horizontalalignment = 'center')

ax2.annotate(METADICT['bellingshausen']['name'], (0.54, 0.65), xycoords = 'axes fraction', horizontalalignment = 'center')


m.plot([-58.86, -58.76], [-62.22, -62.315], color = 'k', linewidth = 1, latlon = True, zorder = 3)
...
m2 = Basemap(width = 12000000, height = 12000000, resolution = 'h', projection = 'laea',
             lat_ts = -60, lat_0 = -90, lon_0 = 0, ax = ax1)

m2.drawparallels(np.arange(-80.,81.,20), labels = [False, False, False, False], dashes = [0.01, 0.01], color = 'gray', zorder = 1)
m2.drawmeridians(np.arange(-180.,181.,20), labels = [False, False, True, True], dashes = [0.01, 0.01], color = 'gray', zorder = 1)
...
m2.fillcontinents(color = 'silver', zorder = 2)

bx, by = m2(m.boundarylons, m.boundarylats)

xy = list(zip(bx, by)) # Getting right map's corners positions (lon and lat) to draw the polygon above

mapboundary = Polygon(xy, facecolor = 'grey', linewidth = 1, alpha = 0.3, fill = False, hatch = 'xxxxxxxx', zorder = 3)

m2.ax.add_patch(mapboundary)


m2.scatter(METADICT['halley']['lon'], METADICT['halley']['lat'],
          s = 55, latlon = True, marker = 'o', color = 'k', linewidths = 1.5, zorder = 3)


m2.scatter([ METADICT['marion']['lon'],       METADICT['amsterdam']['lon'], METADICT['hobart']['lon'],
            METADICT['christchurch']['lon'], METADICT['valdivia']['lon'],  METADICT['gough']['lon'],
            METADICT['novolazara']['lon'],   METADICT['mawson']['lon'],    METADICT['mirny']['lon'],
            METADICT['casey']['lon'],        METADICT['dumont']['lon'],    METADICT['faraday']['lon'] ],

          [ METADICT['marion']['lat'],       METADICT['amsterdam']['lat'], METADICT['hobart']['lat'],
            METADICT['christchurch']['lat'], METADICT['valdivia']['lat'],  METADICT['gough']['lat'],
            METADICT['novolazara']['lat'],   METADICT['mawson']['lat'],    METADICT['mirny']['lat'],
            METADICT['casey']['lat'],        METADICT['dumont']['lat'],    METADICT['faraday']['lat'] ],
          s = 55, latlon = True, marker = 'P', color = 'k', linewidths = 1.5, zorder = 3)


ax1.annotate('(a)', (0.01, 0.97), xycoords = 'axes fraction', fontsize = 12)

ax1.annotate(METADICT['halley']['name'], (0.385, 0.62), xycoords = 'axes fraction', horizontalalignment = 'center')
...
ax1.annotate(METADICT['valdivia']['name'], (0.07, 0.585), xycoords = 'axes fraction', horizontalalignment = 'center')
...
ax1.annotate(METADICT['faraday']['name'], (0.26, 0.56), xycoords = 'axes fraction', horizontalalignment = 'center')
